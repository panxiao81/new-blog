...
在一个简单的计算器上，我们通常要先将 4.99 和 1.06 相乘，将值保存到一个临时变量 $A_1$ 中，然后将 5.99 和 6.99 相加，再次保存进 $A_1$ 中，再将 6.99 和 1.06 相乘，将值保存到一个临时变量 $A_2$ 中，最后将 $A_1$ 和 $A_2$ 相加，得到最终结果。我们可以将操作顺序写作如下的表达式：
